<!-- ===================================================================
  ...
  使い方:
    1. このファイルを docs/embed-notice-kamajan.md にコピー。
    2. 下の kamajan と 釜雀（かまじゃん） を全置換。
...
imapp (https://imapp.shop) に 釜雀（かまじゃん） を **iframe で埋め込んで配信**したい。
...
- [ ] `app/games/kamajan/<Name>.tsx`: `GameEmbedAdapter` を 1 つ宣言（標準なら hostSource:'imappt' / configType:'config' / terminalTypes:['game_over']）
- [ ] `page.tsx` + `<Name>Client.tsx`（結果ダイアログ・スコア保存）
- [ ] `/api/games/kamajan/score` + `/checkin`
- [ ] migration: `kamajan_scores` テーブル / `kamajan_leaderboard` / `_my_stats` RPC / `shop_public_by_qr` に `kamajan_enabled` 列
- [ ] env `NEXT_PUBLIC_<X>_GAME_URL` / `_ORIGIN`、`lib/games/maintenance.ts` フラグ
- [ ] `/games`・`/admin/games`・`/subscribe` の一覧に追加
- [ ] 配信側 `_headers` に `/kamajan/*` ブロック、README 配信テーブルに追記

## 5. 完了の定義 / 連絡

ゲーム側 §3 を満たしたビルドをデプロイしたら、imapp に
**「kamajan 連携実装デプロイ済み」**と伝えてください。imapp 側 §4 を実施して公開します。
...
自分宛ての指示は同 docs/embed-notice-kamajan.md を参照。
