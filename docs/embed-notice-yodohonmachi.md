<!-- embed-onboarding-TEMPLATE.md から生成（2026-08-08） -->


# 通達: 淀本町商店街大作戦 を imapp に連動させる依頼
...
imapp (https://imapp.shop) に 淀本町商店街大作戦 を **iframe で埋め込んで配信**したい。
...
- game-key（＝配信パス）: **`yodohonmachi`**（`games.imapp.shop/yodohonmachi/`）
- game→host の source: **`'yodohonmachi'`**

## 3. ゲーム側チェックリスト（EMBED-CONTRACT §1〜§3 準拠）

- [ ] **単独リポジトリで開発している**（1 タイトル = 1 git リポジトリ・GitHub private・
      リポジトリ名は `yodohonmachi` と同名。他ゲームとの相乗り禁止 → EMBED-CONTRACT §0）。
- [ ] 起動 URL の `?embed=1&imapp=1&player=&hostOrigin=` を読む。`hostOrigin` を保持。
- [ ] 初期化完了で `parent.postMessage({source:'yodohonmachi', type:'ready'}, hostOrigin)`。
- [ ] host からの `{source:'imappt', type:'config', payload:{player, difficulty?}}` を受信。
      `event.origin === hostOrigin` を検証。`player` を反映。
- [ ] 1ラウンド終了で `{source:'yodohonmachi', type:'game_over', payload:{...スコア...}}` を送信。
      （`result` 等の別名にしない）
...
- [ ] **配信規約（games-imapp-shop README「配信規約」参照）に従う**:
      ...
      `_headers` には `/yodohonmachi/assets/*` の1ルールだけ追加し、
...
- [ ] `app/games/yodohonmachi/<Name>.tsx`: `GameEmbedAdapter` を 1 つ宣言（標準なら hostSource:'imappt' / configType:'config' / terminalTypes:['game_over']）
- [ ] `page.tsx` + `<Name>Client.tsx`（結果ダイアログ・スコア保存）
- [ ] `/api/games/yodohonmachi/score` + `/checkin`
- [ ] migration: `yodohonmachi_scores` テーブル / `yodohonmachi_leaderboard` / `_my_stats` RPC / `shop_public_by_qr` に `yodohonmachi_enabled` 列
- [ ] env `NEXT_PUBLIC_<X>_GAME_URL` / `_ORIGIN`、`lib/games/maintenance.ts` フラグ
- [ ] `/games`・`/admin/games`・`/subscribe` の一覧に追加
- [ ] 配信側 README 配信テーブルに追記、`_headers` に `/yodohonmachi/assets/*` の1ルールを追加
      （README「配信規約」参照。`scripts/check-headers.mjs` が追加漏れを検出する）

## 5. 完了の定義 / 連絡

ゲーム側 §3 を満たしたビルドをデプロイしたら、imapp に
**「yodohonmachi 連携実装デプロイ済み」**と伝えてください。imapp 側 §4 を実施して公開します。
...
自分宛ての指示は同 docs/embed-notice-yodohonmachi.md を参照。
