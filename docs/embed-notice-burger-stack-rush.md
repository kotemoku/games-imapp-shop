...
| postMessage 送信先 | ⚠️ `'*'`（全オリジンへ送信） | **`hostOrigin`（起動 URL の値に固定）** |

> ⚠️ **追加のセキュリティ指摘**: 配信中バンドルは `parent.postMessage(payload, '*')` を
> 使用しており、EMBED-CONTRACT §3「`'*'` 禁止」に違反しています。任意のページに
> 埋め込まれた場合にスコア payload を横取りされ得ます。正規化に合わせ、送信先を
> 起動 URL の `hostOrigin` パラメータの値に固定してください（imapp 側は既に付与予定）。
